...
if ARGV.length == 5
  ...
  # region_x, region_z, chunk_index, section_y, block_index
  loc = MCAFile.block_to_mca(*args)
else
  loc = MCAFile.coords_to_mca(ARGV[0].to_f, ARGV[2].to_f, ARGV[1].to_f)
end

ap loc

exit 0 unless opts['dir']

mca = MCAFile.new(File.join(opts['dir'], loc[:mcafile]))

puts "----------------------------------------------------------------"

idx = loc[:chunk_index]
puts "Chunk location: #{mca.locations[idx][0]}"
puts "Sector count: #{mca.locations[idx][1]} (#{4096 * mca.locations[idx][1]} bytes)"
puts "Timestamp: #{Time.at(mca.timestamps[idx])}"

chunk_data = mca.chunk(idx)
puts "Chunk real length: #{chunk_data[0]} bytes"
puts "Compression type: #{chunk_data[1]}"


io = StringIO.new(chunk_data[2])

(chunk_name, chunk) = NBTFile.load(io)
puts "Chunk name: #{chunk_name}"

sections = chunk['Level']['Sections']
puts "Section length: #{sections.length}"
section = sections[loc[:section_y]]

puts "Y: #{section['Y']}"
puts

bidx = loc[:block_index]
block_id = section['Blocks'][bidx].unpack('C')[0]
block_data = MCAFile.halfbyte(section['Data'], bidx)
puts "Block ID: #{block_id}"
puts "Data: #{block_data}"

key = block_data == 0 ? "#{block_id}" : "#{block_id}:#{block_data}"
puts "Block: #{block_ids[key]}"

puts "SkyLignt: #{MCAFile.halfbyte(section['SkyLight'], bidx)}"
puts "BlockLignt: #{MCAFile.halfbyte(section['BlockLight'], bidx)}"

puts

puts "--- Blocks / Height Map / BlockLight / SkyLight ---"
...
sections.each do |section|
  16.times do |o|
    ...
    16.times do |i|
      16.times do |j|
        idx = o * 256 + i * 16 + j
        print '%02x ' % section['Blocks'][idx].unpack('C')[0]
      end
      puts
    end
    ...
    16.times do |i|
      16.times do |j|
        idx = o * 256 + i * 16 + j
        print MCAFile.halfbyte(section['Data'], idx).to_s(16)
      end
      print '   '
      16.times do |j|
        idx = o * 256 + i * 16 + j
        print MCAFile.halfbyte(section['BlockLight'], idx).to_s(16)
      end
      print '   '
      16.times do |j|
        idx = o * 256 + i * 16 + j
        print MCAFile.halfbyte(section['SkyLight'], idx).to_s(16)
      end
      puts
    end
    puts
